...
function parseDateInput(value) {
  ...
  if (parsedDate.getUTCFullYear() !== Number(year) || parsedDate.getUTCMonth() !== Number(month) - 1 || parsedDate.getUTCDate() !== Number(day)) return ''
...
const dateLabel = computed(() => {
  if (form.dateFrom && form.dateTo) return `${formatDateForDisplay(form.dateFrom)} a ${formatDateForDisplay(form.dateTo)}`
  ...
  if (form.date) return props.filters.datas.find((option) => option.value === form.date)?.label ?? form.date
  ...
  <form class="search-panel" @submit.prevent="search">
    ...
    <div class="quick-filters">
      <div class="filter-control date-filter">
        <button class="filter-trigger" type="button" :disabled="props.filtersLoading" :aria-expanded="dateMenuOpen" @click="toggleDateMenu">
        ...
        <div v-if="dateMenuOpen" class="date-menu">
          ...
          <label>Atalho
            <select :value="form.date" @change="selectDatePreset">
              ...
              <option v-for="option in props.filters.datas" :key="option.value" :value="option.value">{{ option.label }}</option>
            </select>
          </label>
          <label>De
            <span class="date-input">
              <input class="date-display" :value="dateInputs.from" type="text" inputmode="numeric" placeholder="dd/mm/yyyy" maxlength="10" @click="openDatePicker('from')" @input="updateDateInput('from', $event)" />
              <input :ref="(element) => datePickerRefs.from = element" class="date-picker" :value="form.dateFrom" lang="pt-BR" type="date" aria-label="Data inicial" @change="selectNativeDate('from', $event)" />
            </span>
          </label>
          <label>Até
            <span class="date-input">
              <input class="date-display" :value="dateInputs.to" type="text" inputmode="numeric" placeholder="dd/mm/yyyy" maxlength="10" @click="openDatePicker('to')" @input="updateDateInput('to', $event)" />
              <input :ref="(element) => datePickerRefs.to = element" class="date-picker" :value="form.dateTo" lang="pt-BR" type="date" aria-label="Data final" @change="selectNativeDate('to', $event)" />
            </span>
          </label>
      ...
      <div class="filter-control select-filter">
        <button class="filter-trigger" type="button" :disabled="props.filtersLoading" :aria-expanded="filterMenuOpen === 'area'" aria-haspopup="listbox" @click="toggleFilterMenu('area')">
        ...
        <div v-if="filterMenuOpen === 'area'" class="filter-menu" role="listbox" aria-label="Áreas">
          ...
          <button v-for="option in props.filters.areas" :key="option.acronym" type="button" :class="{ selected: form.area === option.acronym }" @click="selectFilter('area', option.acronym)">{{ option.acronym }} • {{ option.name }}</button>
        </div>
      </div>
      <div class="filter-control select-filter">
        <button class="filter-trigger" type="button" :disabled="props.filtersLoading" :aria-expanded="filterMenuOpen === 'type'" aria-haspopup="listbox" @click="toggleFilterMenu('type')">
        ...
        <div v-if="filterMenuOpen === 'type'" class="filter-menu" role="listbox" aria-label="Tipos de documento">
          ...
          <button v-for="option in props.filters.tipos" :key="option.code" type="button" :class="{ selected: form.type === option.code }" @click="selectFilter('type', option.code)">{{ option.code }} • {{ option.name }}</button>
        </div>
      </div>
    </div>
...
.date-picker { position: absolute; inset: 0; z-index: 0; padding: 0 !important; border: 0 !important; opacity: 0; pointer-events: none; }
