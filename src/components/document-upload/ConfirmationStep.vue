...
const documentType = computed(() => {
  ...
  return type ? `${type.acronym} - ${type.name}` : EMPTY
})
const discipline = computed(() => {
  const d = store.selectedDiscipline
  return d ? `${d.acronym} - ${d.name}` : EMPTY
})
    ...
    <dl class="summary">
      <div class="summary-row summary-row-header">
        ...
        <div class="summary-cell summary-meta">
          <KeyValue label="Versão atual" layout="row">{{ store.form.revision }}</KeyValue>
      ...
      <div class="summary-row summary-row-three">
        ...
        <div class="summary-cell">
          <KeyValue label="Confidencialidade">
            <Badge v-if="confidentiality" :variant="confidentiality.value">{{ confidentiality.label }}</Badge>
      ...
      <div class="summary-row summary-row-two">
        <div class="summary-cell">
          <KeyValue label="Tags relacionadas">
            <span v-if="store.form.areas.length" class="summary-tags">
              <Badge v-for="area in store.form.areas" :key="area">{{ area }}</Badge>
            </span>
            <template v-else>{{ EMPTY }}</template>
          </KeyValue>
        </div>
